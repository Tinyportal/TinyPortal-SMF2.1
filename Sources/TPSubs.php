...
function tp_collectArticleIcons()
{
	...
	if($handle = opendir($boarddir.'/tp-files/tp-articles/icons')) 
	{
		while (false !== ($file = readdir($handle))) 
		{
			if($file != '.' && $file != '..' && $file != '.htaccess' && $file != 'TPnoimage.png' && in_array(strtolower(substr($file, strlen($file) -4, 4)), array('.gif', '.jpg', '.png')))
			{
				$context['TPortal']['articons']['icons'][] = array(
					'id' => $count,
					'file' => $file,
...
function tp_recentTopics($num_recent = 8, $exclude_boards = null, $output_method = 'echo')
{
	...
	$request = $smcFunc['db_query']('', '
		SELECT
			...
			IFNULL(mem.real_name, m.poster_name) AS posterName,  mem.email_address, mem.avatar
...
function tpattach()
{
	...
	$context['utf8'] = $context['character_set'] === 'UTF-8' && (strpos(strtolower(PHP_OS), 'win') === false);
...
function tp_getblockstyles()
{
	return array(
		...
		'4' => array(
			...
			'code_bottom' => '</div><span class="lowerframe"><span></span></span>',
		),
	);
}
